...
function renderWeatherDetails(json){        
    ...
    //wind speed
    document.querySelector('.wind-speed').textContent = 'Wind Speed:'+ ' ' + json.wind.speed + 'm/s';
    //How clowdy it is
    document.querySelector('.how-cloudy').textContent = 'clouds:'+ ' ' + json.clouds.all+'%';  
    //When sunrise and sunset is
    document.querySelector(".sunrise").textContent = `Sunrise: ${(new Date(json.sys.sunrise * 1000)).toLocaleTimeString()}`;
    ...
    function renderLocationOnGoogleMap(lat, lng) {
      ...
      var marker = new google.maps.Marker({   //my time to create unique feature
        position: { lat, lng },
        map: map,
       // zoom: 10,
        label: {
...
document.querySelector('.btn2').addEventListener('click', () => {
  navigator.geolocation.getCurrentPosition(position => {
    ...
    const BASE_URL =
      ...
      "&appid=" +
      API_KEY;
    fetch(BASE_URL).then(response => response.json()).then(json => renderWeatherDetails(json))
  })
